...
/*
  Camera zoom toward pill region.
  These values are tuned to:
  - center the pill cluster
  - scale viewport into it
*/
...
.toast-card {
	...
	padding: clamp(18px, 2.2vh, 32px) clamp(18px, 1.8vw, 28px) clamp(14px, 1.8vh, 24px);
	display: flex;
	flex-direction: column;
	gap: clamp(12px, 1.6vh, 22px);
	box-shadow: 0 4px 32px rgba(58, 47, 43, 0.13)
}
...
.toast-cta {
	...
	border: none;
	border-radius: 999px;
	padding: clamp(6px, 0.7vh, 10px) clamp(14px, 1.2vw, 20px);
	cursor: pointer;
	display: flex;
	align-items: center;
	gap: 0.3em;
	...
	box-shadow: 0 2px 8px rgba(58, 47, 43, 0.08)
}

.toast-cta:hover {
	transform: scale(1.07);
	box-shadow: 0 4px 16px rgba(58, 47, 43, 0.14)
}
.selection-viewport{
  position:absolute;
  inset:0;
...
.selection-viewport.selection-viewport-active{
  transform:scale(1);
...
@keyframes shrink0 {
	from {
		transform: translate(calc(50vw - 4vw - 1.3vw), calc(50vh - 4.5vh - 1.3vw)) scale(18)
	}
...
@keyframes shrink1 {
	from {
		transform: translate(calc(50vw - 4vw - 3.3vw - 1.3vw), calc(50vh - 4.5vh - 1.3vw)) scale(18)
	}
...
@keyframes shrink2 {
	from {
		transform: translate(calc(50vw - 4vw - 6.6vw - 1.3vw), calc(50vh - 4.5vh - 1.3vw)) scale(18)
	}
...
@keyframes circleToCorner {
	...
	to {
		transform:
			translate(33vw, 34vh) scale(1.15);

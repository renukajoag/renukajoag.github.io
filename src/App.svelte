<script>
  ...
  // States: 'landing' | 'collapsing' | 'circles' | 'expanded' | 'deep'
  let state         = 'landing';
  let expandedIndex = null;
  let expandOrigin  = null;
  let hoveredIndex  = null;
  let showSkipToast = false;
  let showFuturePage = false;

  // During 'deep' entry/exit we need the WhoIAm bg to show
  // WHILE the circle is still animating — so we keep a separate flag
  // that turns on slightly before state becomes 'deep' and off after exit.
  let deepBgVisible = false;

  // A separate flag to play the circle-shrink-to-corner animation
  // without being in full 'deep' state yet
  let circleGoingToCorner = false;   // circle animating toward corner
  let circleComingFromCorner = false; // circle animating back from corner
  ...
  function handleWheel(e) {
    ...
    if (state === 'deep') return; // WhoIAm handles its own scroll

    if (showFuturePage) {
      if (e.deltaY < -30) { showFuturePage = false; lockScroll(600); }
      return;
    }
  ...
  function goToCircles() {
    state = 'circles'; expandedIndex = null; expandOrigin = null; lockScroll(900);
  }
  ...
  $: isCirclesVisible = ['circles','expanded','collapsing','deep'].includes(state) || circleGoingToCorner || circleComingFromCorner;

  // Show the expanded big circle when: expanded state, or mid-transition
  $: showBigCircle = (state === 'expanded' || circleGoingToCorner) && expandedIndex !== null;

  // In deep or transitioning: show the corner dot for slot 0 (Who I am)
...
<div class="root">
  ...
  <!-- ════════ LANDING ════════ -->
  <div class="landing-page" class:hidden={state !== 'landing' && state !== 'collapsing'}>
  ...
  <!-- ════════ PILL-AREA ════════ -->
  <div class="pill-area" class:pill-area-open={isCirclesVisible} class:pill-area-closing={state === 'collapsing'}></div>

  <!--
    ════════ WHO I AM — rendered BELOW the circles layer ════════
    z-index: 5 — above circles-page (z:2) but below the travelling circle (z:12)
    The gradient background is always present here.
    It becomes visible as soon as deepBgVisible is true, which happens
  ...
  <!-- ════════ CIRCLES PAGE ════════ -->
  <div class="circles-page" class:circles-page-visible={isCirclesVisible}>

    <!-- Corner slots (expanded view OR deep) -->
    {#if (state === 'expanded' || state === 'deep' || circleGoingToCorner || circleComingFromCorner) && expandedIndex !== null}
      <div class="corner-pill-slots">
        ...
        {#if slotIdx === expandedIndex
            ...
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            <!-- svelte-ignore a11y-no-static-element-interactions -->
            <div class="corner-pill-slot" on:click={() => state === 'deep' && handleCornerPillClick(slotIdx)}>
              <div class="corner-circle {circleGoingToCorner ? 'corner-shrink-0' : circleComingFromCorner ? 'corner-grow-0' : ''}"></div>
            </div>
          {:else if slotIdx !== expandedIndex}
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            <!-- svelte-ignore a11y-no-static-element-interactions -->
            <div class="corner-pill-slot" on:click={() => handleCornerPillClick(slotIdx)} title={sections[slotIdx].label}>
    ...
    <!-- Big expanded circle — only in 'expanded' state or while going to corner -->
    {#if showBigCircle}
      {#key expandedIndex}
        <div class="expanded-scene" class:circle-exit={circleGoingToCorner}>
          <div
            class="big-expanded-circle {originClass(expandOrigin)} {circleGoingToCorner ? 'circle-to-corner' : ''}"
            style="background:{sections[expandedIndex].color}"
          >
            {#if !circleGoingToCorner}
              <div class="expanded-items">
                {#each (sections[expandedIndex].hasDeepSection
                  ? ['Who I am','What makes me, me','What I care about','Outside of design','Right now']
                  : ['Brand identity','Editorial design','Illustration','Motion','Packaging']) as item, idx}
                  <span class="expanded-item" style="animation-delay:{0.25+idx*0.1}s">{item}</span>
                {/each}
              </div>
...
<style>
  ...
  .root{width:100vw;height:100vh;position:relative;overflow:hidden;background:#f2eded}

  .future-page{position:absolute;inset:0;z-index:20;background:#f2eded;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.5s ease forwards}
  .future-hint{font-family:'Trispace',monospace;font-stretch:112.5%;font-size:1vw;color:#3A2F2B;letter-spacing:0.15em;opacity:0.5}

  .landing-page{position:absolute;inset:0;z-index:4;transition:opacity 0.5s ease}
  ...
  .pill-area{position:absolute;top:calc(4.5vh + 1.3vw);left:8.6vw;transform:translate(-50%,-50%);width:calc(3*2.6vw + 2*0.7vw);height:2.6vw;border-radius:999px;background:transparent;z-index:3;pointer-events:none;transition:width 0.85s cubic-bezier(0.4,0,0.2,1),height 0.85s cubic-bezier(0.4,0,0.2,1),top 0.85s cubic-bezier(0.4,0,0.2,1),left 0.85s cubic-bezier(0.4,0,0.2,1),border-radius 0.7s cubic-bezier(0.4,0,0.2,1)}
  .pill-area.pill-area-open{top:50vh;left:50vw;width:100vw;height:100vh;border-radius:0}
  .pill-area.pill-area-closing{top:calc(4.5vh + 1.3vw);left:8.6vw;width:calc(3*2.6vw + 2*0.7vw);height:2.6vw;border-radius:999px}

  /* circles-page z:2 sits behind WhoIAm (z:5) which sits behind travelling circle (z:12) */
  .circles-page{position:absolute;inset:0;z-index:2;background:#f2eded;opacity:0;pointer-events:none;transition:opacity 0.4s ease 0.55s;display:flex;align-items:center;justify-content:center}
  .circles-page.circles-page-visible{opacity:1;pointer-events:all}
  ...
  /* shrink = from center toward slot; grow = reverse */
  ...
  .ci-0{animation:circleFromCorner 0.7s cubic-bezier(0.16,1,0.3,1) 0.55s both}
  .ci-1{animation:circleFromCorner 0.7s cubic-bezier(0.16,1,0.3,1) 0.65s both}
  .ci-2{animation:circleFromCorner 0.7s cubic-bezier(0.16,1,0.3,1) 0.75s both}
  ...
  /* When going to corner: the big circle shrinks from center to slot-0 position */
  .big-expanded-circle.circle-to-corner{
    animation: circleToCorner 0.65s cubic-bezier(0.4,0,0.2,1) forwards !important;
  }
...
@keyframes circleToCorner {
  ...
  @keyframes circleFromCorner{from{opacity:0;transform:translate(-38vw,-42vh) scale(0.08)}to{opacity:1;transform:translate(0,0) scale(1)}}
